...
<section data-slice-type={slice.slice_type} data-slice-variation={slice.variation} class="px-4 md:px-6">
	<div class="mx-auto w-full max-w-7xl"> 
		<div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
			...
			<div class="col-start-1 md:col-span-1 md:row-span-1 md:col-start-1">
				<h1 class="mb-2 md:mb-8 text-[clamp(3rem,20vmin,13rem)] font-extrabold leading-none tracking-tighter text-nowrap" aria-label={slice.primary.firstname + ' ' + slice.primary.lastname}>
					{#if first_name_letters.length && last_name_letters.length}
					<span  class="block text-neutral-50">
						{#each first_name_letters as letter}
						<span class ="name-animation inline-block opacity-100">{letter}</span>
						{/each}
					</span>
					{/if}
					<span class="block text-neutral-300 -mt-[.2em]">
						{#each last_name_letters as letter}
						<span class ="name-animation inline-block opacity-100">{letter}</span>
						{/each}					
					</span>
				</h1>
				<span class="job-title block bg-gradient-to-tr from-neutral-50 via-neutral-300 to-neutral-50 bg-clip-text text-transparent text-2xl font-bold uppercase tracking-[.2em] md:text-4xl opacity-100">
