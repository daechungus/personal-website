		...
		<div class="grid min-h-[65vh] grid-cols-1 items-center md:grid-cols-2">
			<div class="col-start-1 md:col-span-1 md:row-span-1 md:col-start-1 order-2 md:order-1">
				<h1 class="mb-2 md:mb-8 text-[clamp(3rem,20vmin,11rem)] font-extrabold leading-none tracking-tighter text-nowrap" aria-label={slice.primary.firstname + ' ' + slice.primary.lastname}>
					...
					<span class="block opacity-95 transition-colors duration-300 {$theme === 'opus' ? 'text-white' : 'text-matcha-600'}">
						{#each first_name_letters as letter}
						<span class="name-animation inline-block opacity-100">{letter}</span>
						{/each}
					</span>
					{/if}
					<span class="block -mt-[.2em] opacity-90 transition-colors duration-300 {$theme === 'opus' ? 'text-white/70' : 'text-matcha-500'}">
						{#each last_name_letters as letter}
						<span class="name-animation inline-block opacity-100">{letter}</span>
						{/each}
					</span>
				</h1>
				<span class="px-2 job-title block bg-clip-text text-transparent text-xl font-bold uppercase tracking-[.2em] md:text-4xl opacity-100 transition-all duration-300
					{$theme === 'opus' ? 'bg-gradient-to-tr from-white/30 via-white/80 to-white/30' : 'bg-gradient-to-tr from-sage-300 via-matcha-500 to-sage-300'}">
